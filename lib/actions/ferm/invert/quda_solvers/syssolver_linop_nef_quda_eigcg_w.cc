...
#include "actions/ferm/invert/quda_solvers/syssolver_linop_nef_quda_eigcg_w.h"
...
namespace Chroma
{
  namespace LinOpSysSolverQUDANEFEIGCGEnv
  {
    ...
    {
      //! Name to be used
      const std::string name("QUDA_NEF_EIGCG_INVERTER");
    ...
    {
      return new LinOpSysSolverQUDANEFEIGCG(A, state,SysSolverQUDANEFEIGCGParams(xml_in, path));
    }
  ...
  SystemSolverResults_t LinOpSysSolverQUDANEFEIGCG::qudaInvert(const multi1d<T>& chi_s, multi1d<T>& psi_s) const{
